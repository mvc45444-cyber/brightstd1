...
export const metadata = {
  title: 'Blog | Iron Peak Solutions',
      ...
      <section className="bg-white border-b border-slate-200 py-16 md:py-24 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 md:px-6 text-center max-w-4xl relative z-10">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 mb-6 tracking-tight">
            Latest <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-600 to-orange-600">Articles</span>
          </h1>
          ...
          >
            <div className="md:grid md:grid-cols-2 md:gap-0">
              ...
              <div className="p-8 md:p-10 flex flex-col justify-center">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-50 text-amber-700 text-xs font-medium mb-4 w-fit border border-amber-100">
                  Featured Article
                </div>
                ...
                <h2 className="text-2xl md:text-3xl font-bold text-slate-900 mb-4 group-hover:text-amber-600 transition-colors leading-tight">
                  {featuredPost.title}
                </h2>
        ...
        {otherPosts.length > 0 && (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {otherPosts.map((post) => (
              <article
                key={post.slug}
                className="group flex flex-col h-full bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-lg hover:border-amber-200 transition-all"
              >
                ...
                <div className="p-6 flex-grow flex flex-col">
                  ...
                  <h2 className="text-lg font-bold text-slate-900 mb-3 group-hover:text-amber-600 transition-colors leading-tight line-clamp-2">
                  ...
                  <Link
                    href={`/blog/${post.slug}`}
                    className="text-amber-600 font-semibold text-sm flex items-center gap-1 group-hover:gap-2 transition-all mt-auto"
                  >
                    Read Article <ArrowRight className="w-4 h-4" />
                  </Link>
                </div>
      ...
      <section className="container mx-auto px-4 md:px-6 max-w-6xl">
        <div className="bg-slate-900 rounded-2xl p-8 md:p-12 text-center text-white">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link
              href="/reviews"
              className="bg-amber-600 text-white font-bold py-3 px-8 rounded-full hover:bg-amber-700 transition-colors"
            >
              Browse Reviews
            </Link>
            <Link
